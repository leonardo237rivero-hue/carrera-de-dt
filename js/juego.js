...
function portada(){
  S=nuevo();
  const demo=armarOnce(generarPlantel(70),"4-3-3");
  $("#s-portada").innerHTML=`
    <div style="margin:26px 0 18px">
      <h2 class="disp" style="font-size:46px">Carrera de DT</h2>
      <p class="mini" style="margin-top:6px">Seis temporadas. Vos elegís cómo para el equipo.</p>
    </div>
    ${cancha(demo,{mostrarRating:false,alto:250})}
    <div class="acciones"><button class="btn" onclick="identidad()">Empezar</button></div>`;
  ir("portada");
}
...
/* ==========================================================
   ...
   ========================================================== */
function identidad(){
  tmp={arq:null,sis:null,est:null,hincha:"Peñarol"};
  pintarIdentidad(); ir("identidad");
}
function pintarIdentidad(){
  const listo=tmp.arq&&tmp.sis&&tmp.est;
  $("#s-identidad").innerHTML=`
    <h2 class="disp">Quién sos</h2>
    ...
    <h3 class="disp">Tu perfil</h3>
    ${ARQ.map(a=>`<button class="card ${tmp.arq===a.id?'sel':''}" onclick="setArq('${a.id}')">
      <b>${a.n}</b><div class="sub">${a.d}</div>
      <div class="efecto">${a.ef.join(" · ")}</div>
      <div class="costo">${a.co}</div></button>`).join("")}
    ...
    <h3 class="disp">Tu sistema de cabecera</h3>
    ...
    <h3 class="disp">Tu modelo de juego</h3>
    ...
    <div class="acciones">
      <button class="btn" onclick="verOfertas()" ${listo?'':'disabled'}>Buscar club</button>
      <button class="btn sec" onclick="portada()">Volver</button></div>`;
}
...
function verOfertas(){
  ...
  $("#s-ofertas").innerHTML=`
    <h2 class="disp">Quién te quiere</h2>
    ...
    ${S.ofertas.map((o,i)=>`
      <button class="card" onclick="firmar(${i})">
        <div style="display:flex;gap:12px;align-items:center">${escudo(o.club,40)}
          <div><b>${o.club.n}</b>
          <div class="sub">${o.club.p} · ${DIVISIONES[o.div].n} · plantel nivel ${o.club.niv}/5</div></div></div>
...
function firmar(i){
  ...
  S.rng=mul32(hashS(S.club.n+S.arq.id+S.sistemaPref+Date.now()));
...
function verPlantel(){
  ...
  const supl=S.plantel.filter(j=>!once.some(o=>o.jug===j)).sort((x,y)=>y.rt-x.rt);
  $("#s-plantel").innerHTML=`
    <h2 class="disp">Tu plantel</h2>
    <p class="mini">Parado con ${f}. Los que llegaron hace poco salen marcados en ámbar.
    ${fuera?`<span style="color:var(--ambar)">${fuera} juega${fuera>1?'n':''} fuera de puesto y pierde${fuera>1?'n':''} 10 puntos.</span>`:""}</p>
    ${cancha(once,{mostrarNombre:true,alto:340})}
    ${barras(a)}
    <h3 class="disp">Los once</h3>
    <table><tr><th>Puesto</th><th>Jugador</th><th>Nivel</th><th>Edad</th><th></th></tr>
    ${once.map(o=>`<tr>
      <td>${ROL[o.rol].n}</td>
      <td>${o.jug.nom}${o.jug.nuevo?' <span style="color:var(--ambar)">nuevo</span>':''}${S.fijos.includes(o.jug)?' <span style="color:var(--verde)">fijo</span>':''}</td>
      <td><b>${o.rt}</b>${o.fuera?' <span style="color:var(--ambar)">fuera de puesto</span>':''}</td>
      <td>${o.jug.ed}</td>
      <td>${S.fijos.includes(o.jug)?`<button class="mini-btn" onclick="soltar(${S.plantel.indexOf(o.jug)})">soltar</button>`:''}</td></tr>`).join("")}
    </table>
    <h3 class="disp">Suplentes</h3>
    <p class="mini">Podés meter a mano a cualquiera. Si lo fijás, entra sí o sí en su puesto.</p>
    <table><tr><th>Jugador</th><th>Puesto</th><th>Nivel</th><th>Edad</th><th></th></tr>
    ${supl.map(j=>`<tr>
      <td>${j.nom}${j.nuevo?' <span style="color:var(--ambar)">nuevo</span>':''}</td>
      <td>${grupoLbl(j.g)}</td><td>${j.rt}</td><td>${j.ed}</td>
      <td><button class="mini-btn" onclick="fijar(${S.plantel.indexOf(j)})">poner de titular</button></td></tr>`).join("")}
    </table>
    <div class="acciones"><button class="btn" onclick="abrirMercado()">
      ${S.esSeleccion?"Ir a la convocatoria":(S.division<3?"Ir a captación":"Ir al mercado de pases")}</button></div>`;
  ir("plantel");
}
function fijar(idx){
  ...
  $("#s-mercado").innerHTML=`
    ...
    <div class="grid3">${S.cartas.map((k,i)=>{
      ...
      return `<button class="jug" ${k.estado!=="libre"||costo>S.presupuesto?"disabled":""} onclick="piquear(${i})">
        ...
        <div class="ps">${({POR:"Arquero",DEF:"Defensa",MED:"Mediocampo",ATA:"Ataque"})[k.j.g]} · ${k.j.ed} años · ${S.esSeleccion?"convocable":"cuesta "+k.costo}</div>
    ...
    <div class="acciones"><button class="btn" onclick="cerrarMercado()">Cerrar el mercado</button></div>`;
}
...
function cerrarMercado(){clearInterval(S.timer);abrirRuleta();}
...
function pintarRuleta(){
  $("#s-ruleta").innerHTML=`
    ...
    <div class="acciones">
      <button class="btn" id="btnGirar" onclick="girar()" ${tmp.fichas.length===2&&!tmp.girada?'':'disabled'}>Girar</button>
      <button class="btn sec" id="btnSeguirRul" onclick="siguienteDecision()" ${tmp.girada?'':'disabled'}>Seguir</button></div>
    <div id="salida"></div>`;
}
...
function tomar(i){
  const d=S.decs[S.decIdx], o=(S.opsActuales||d[2])[i], [txt,res,dir,hin,ves]=o;
  ...
  if(d[0]==="Táctica"&&res){ // las tácticas mueven el nivel de los jugadores del once
    const f=S.formacion||S.sistemaPref, once=armarOnce(S.plantel,f,S.fijos);
    once.slice(0,6).forEach(x=>{x.jug.rt=clamp(x.jug.rt+(res>0?1:-1),28,94);});
  }
  $("#s-decision").innerHTML=`
    <h2 class="disp">Decisión ${S.decIdx+1} de ${S.decs.length}</h2>
    <div class="ficha-sit"><div class="cat">${d[0]}</div><p>${d[1]}</p></div>
    ...
    <div class="deltas">
      <span class="d ${cd(res)}">Resultados ${sg(res)}</span>
      <span class="d ${cd(dir)}">Directiva ${sg(dir)}</span>
      <span class="d ${cd(hin)}">Hinchada ${sg(hin)}</span>
      <span class="d ${cd(ves)}">Vestuario ${sg(ves)}</span></div>
    <div class="acciones"><button class="btn" onclick="S.decIdx++;siguienteDecision()">Seguir</button></div>`;
  cabecera();
}
...
function irPlanteo(){
  ...
  const dif=(S.temporada-1)*2+(rivClub.niv-S.club.niv)*4;
...
function pintarPlanteo(){
  ...
  $("#s-planteo").innerHTML=`
    <h2 class="disp">Cómo los parás</h2>
    <div style="display:flex;gap:10px;align-items:center;margin-bottom:4px">${escudo(rv.n,32)}
      ...
      <div class="mini">${S.compe} · sale con ${rv.formacion}, ${ESTILO_LBL[rv.estilo]}</div></div></div>
    ...
    <h3 class="disp">Probá otro sistema</h3>
    ...
    <div class="acciones">
      <button class="btn" onclick="jugar()">Jugar la temporada</button>
...
function jugar(){
  ...
  S.ajustePos=ap;
  const pref=(S.formacion===S.sistemaPref?1.03:0.98)*(S.estilo===S.estiloPref?1.02:0.99)*ap.mult;
  ...
  if(dif>=12){pos="Campeón";pts=10;dRes=16;dDir=14;dHin=18;sube=true;}
  else if(dif>=5){pos="Subcampeón";pts=7;dRes=9;dDir=8;dHin=9;sube=S.division<3;}
  ...
  pintarResolucion(); ir("resolucion");
}
function pintarResolucion(){
  ...
  $("#s-resolucion").innerHTML=`
    ...
    <details><summary>Ver cómo se resolvió</summary>
      <div class="ln"><span>Nivel del once con ${S.formacion}</span><b>${m.base.toFixed(1)}</b></div>
      <div class="ln"><span>Fit del modelo con el sistema (${FIT_LBL[m.fit]})</span><b>×${m.fit.toFixed(2)}</b></div>
    ...
    <div class="acciones"><button class="btn" onclick="cerrarTemporada()">Cerrar la temporada</button></div>`;
}
...
function cerrarTemporada(){
  ...
  if(S.division<3&&!S.esSeleccion){
    ...
    if(salto>0){
      ...
      if(ascendio) eventos.push(`Te suben a ${DIVISIONES[S.division].n} de ${S.club.n}.`);
    }
  }
  ...
  // despido
  let despedido=false;
  if(S.vars.dir<umbral&&S.temporada<TEMPORADAS){
    ...
    S.club=chico; S.division=3; S.estadio=1;
  ...
  if(ascendio&&S.temporada<TEMPORADAS){ pantallaAscenso(); return; }
  // oferta de seleccion: buen recorrido, en primera, no siendo ya seleccionador
  if(!despedido&&!S.esSeleccion&&S.division===3&&S.temporada>=4&&S.temporada<TEMPORADAS
  ...
  // te vienen a buscar
  if(!despedido&&!S.esSeleccion&&S.temporada<TEMPORADAS&&S.calc.pts>=7&&S.vars.hin>50){
  ...
  // favor al ex club
  let favor=null;
  if(!despedido&&!S.esSeleccion&&S.exClub&&S.presupuesto>45&&S.temporada<TEMPORADAS){
    favor=S.exClub;
  }
  $("#s-club").innerHTML=`
    ...
    :`<div class="acciones"><button class="btn" onclick="siguienteTemporada()">Seguir</button></div>`}`;
  ir("club");
}
...
function pintarAscenso(){
  ...
  $("#s-club").innerHTML=`
    ...
    <p>Pasás a <b>${DIVISIONES[S.division].n}</b> de ${S.club.n}. El plantel de arriba es otro,
    ...
    ${cand.map((j,i)=>`
      <button class="card ${tmp.suben.includes(i)?'sel':''}"
        ...
        <div class="sub">${({POR:"Arquero",DEF:"Defensa",MED:"Mediocampo",ATA:"Ataque"})[j.g]} · ${j.ed} años · nivel ${j.rt}</div>
      </button>`).join("")}
...
function siguienteTemporada(){
  S.temporada++;
  if(S.temporada>TEMPORADAS){final();return;}
  abrirTemporada();
}
...
function final(){
  ...
  const score=Math.round((S.puntos/(TEMPORADAS*10))*70+(prom/100)*30);
  const nota=score>=88?"S":score>=74?"A":score>=58?"B":score>=42?"C":"D";
  const tit=S.titulos.length;
  const ap=apodo();
  window.__res=`Carrera de DT — ${S.nombre}\nNota ${nota} · ${score}/100 · ${ap}\n${tit} título(s) · terminó en ${S.club.n}, ${DIVISIONES[S.division].n}`;
  $("#s-final").innerHTML=`
    <h2 class="disp">Se terminó</h2>
    <div class="tarjeta">
      <div class="nota">${nota}</div>
      <div class="apodo">${ap}</div>
      <div class="mini">${S.nombre} · ${score} de 100</div>
      <hr>
      <div class="fila"><span>Puntos de carrera</span><span>${S.puntos} de ${TEMPORADAS*10}</span></div>
      <div class="fila"><span>Títulos</span><span>${tit?Array(tit).fill(copaSVG(15)).join(" "):"—"} ${tit}</span></div>
      <div class="fila"><span>Terminó en</span><span>${S.club.n}, ${DIVISIONES[S.division].n}</span></div>
    ...
    <h3 class="disp">Tu trayectoria</h3>
    ...
    <div class="acciones">
      <button class="btn" onclick="copiar()">Copiar resultado</button>
      <button class="btn sec" onclick="portada()">Carrera nueva</button></div>
